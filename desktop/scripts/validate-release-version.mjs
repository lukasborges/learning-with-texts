...
export async function validateReleaseVersion(root, expectedTag) {
  ...
  const tauriConfig = await readJson(path.join(root, 'desktop/src-tauri/tauri.conf.json'));
  const cargoToml = await readFile(path.join(root, 'desktop/src-tauri/Cargo.toml'), 'utf8');
  const cargoLock = await readFile(path.join(root, 'desktop/src-tauri/Cargo.lock'), 'utf8');
  const versions = new Map([
    ['package.json', packageJson.version],
    ['package-lock.json', packageLock.version],
    ['package-lock.json root package', packageLock.packages?.['']?.version],
    ['desktop/src-tauri/Cargo.toml', packageVersionFromToml(cargoToml, 'Cargo.toml')],
    [
      'desktop/src-tauri/Cargo.lock',
      packageVersionFromToml(cargoLock, 'Cargo.lock', 'lwt-desktop')
    ],
