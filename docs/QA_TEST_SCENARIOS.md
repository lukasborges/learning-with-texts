...
| --- | --- | --- | --- |
| HOME-01 | First language setup | Launch with a new application-data directory, choose a learning language and a native language, review or edit the two recommended dictionary templates, then submit. | The setup fields and language-pair recommendations are visible directly on Home, both dictionary templates and the learning-to-native translation template are saved before any text exists, and Add content opens with the learning language prefilled. |
...
| LIB-03 | Import text file | Select a UTF-8 `.txt` file, then repeat by dropping it on the text-file widget, and save it. | Click and drag-and-drop both load the content; the selected filename appears in the widget and suggests a title without overwriting a typed title. |
...
| READ-04 | External lookup and translation | Configure valid dictionary/translation URL templates; look up a word and an expression, then translate a complete sentence; also try malformed and non-HTTP templates. | Valid actions open an auxiliary application window with the encoded selection or full sentence; word/expression labels are accurate, and unsafe or malformed templates are not offered. |
...
| READ-06 | Optional local-audio lifecycle | Add a text with valid MP3/WAV audio; use Play/Pause, seek backward/forward 10 seconds, timeline, volume, mute, and playback speeds from 0.5× to 2×; replace it while editing, remove it after confirming, and repeat with no audio. | Audio is optional; valid files up to 50 MB are stored locally, the Reader shows the matching filename and a consistent Libadwaita-style player, every custom control updates the media accessibly, the selected speed is retained while the app remains open, replacement is atomic, removal keeps the text and progress, and texts without audio remain unchanged. |
| READ-07 | Audio validation and backup | Try empty, oversized, unsupported, and content/type-mismatched audio; then export and restore a text with valid audio. | Invalid media is rejected with a clear message before or during persistence, while valid audio survives backup/restore with filename, media type, bytes, and playback intact. |
| READ-07A | Drag-and-drop audio | Drag a valid audio file onto the optional audio widget, save the text, and open the Reader. | The widget highlights while dragging, shows the dropped filename, stores the audio, and the Reader can play it just as when selected through Browse. |
...
| READ-11 | Contextual term pop-up | Open the Reader at wide and narrow sizes, confirm the editor is initially absent, then select different words, repeated words, and an expression; close with its button and Escape. | The text uses the full workspace at every size; the term editor appears as a centered modal only after a term is selected, always shows the current selection, closes accessibly, and does not change the reading position. |
| READ-12 | Fixed reading workspace | Open a long text, scroll from beginning to end, open and close the term pop-up, and inspect the header action. | Only the text region scrolls; title, progress, audio, legend, expression tools, and exactly one Finish lesson action remain in the fixed reading header without viewport-floating overlap. |
| READ-13 | Mobile expression carousel | Save enough expressions to exceed the available width, resize to a narrow phone-sized window, and navigate with touch/trackpad and the Previous/Next controls. | The fixed header remains compact, expressions stay on one horizontal row, navigation reveals every chip without increasing header height, and disabled controls accurately represent the first and last positions. |
...
| CFG-08 | Add a learning language | Open Language settings, choose **Add language**, select or enter a unique learning language, choose a native language, review the recommended dictionaries, then save. | The dialog closes, the new language, two dictionaries, and translation direction are saved; the language is selected for configuration and remains available independently of text creation. |
